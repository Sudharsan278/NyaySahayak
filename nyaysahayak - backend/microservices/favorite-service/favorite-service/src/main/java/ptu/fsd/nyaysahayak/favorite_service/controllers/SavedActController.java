package ptu.fsd.nyaysahayak.favorite_service.controllers;
...
import ptu.fsd.nyaysahayak.favorite_service.services.SavedActService;

@RestController
@RequestMapping("/api/saved-acts")
public class SavedActController {

	@Autowired
	private SavedActService service;
    ...
    public ResponseEntity<?> saveAct(@RequestBody Map<String, Object> request) {
        try {
            ...
            SavedAct savedAct = service.saveActForUser(userEmail, userFirstName, actId);
    ...
    public ResponseEntity<List<SavedAct>> getUserSavedActs(@PathVariable String userEmail) {
        try {
            List<SavedAct> savedActs = service.getSavedActsByUser(userEmail);
    ...
    public ResponseEntity<?> removeSavedAct(@PathVariable String userEmail, @PathVariable int actId) {
        try {
            boolean removed = service.removeSavedAct(userEmail, actId);
            if (removed) {
    ...
    public ResponseEntity<Map<String, Boolean>> isActSaved(@PathVariable String userEmail, @PathVariable int actId) {
        try {
            boolean isSaved = service.isActSavedByUser(userEmail, actId);
    ...
    public ResponseEntity<Map<String, Long>> getSavedActsCount(@PathVariable String userEmail) {
        try {
            long count = service.getSavedActsCountByUser(userEmail);
